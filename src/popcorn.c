/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   popcorn.c                                          :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: mdufaud <mdufaud@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2014/04/30 22:47:41 by mdufaud           #+#    #+#             */
/*   Updated: 2016/01/04 16:50:28 by mdufaud          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"
#include <math.h>
#include <mlx.h>

static int	get_iter(t_fractol *f)
{
	int		iter;
	double	h;

	iter = 0;
	h = 1 + (double)(f->utility_one / 10);
	while (iter < f->iterations && f->real * f->real + f->im * f->im < 4)
	{
		f->real_tmp = f->real;
		f->im_tmp = f->im;
		f->real = f->real - h * (sin(f->im + tan(3 * f->im)));
		f->im = f->im - h * (sin(f->real_tmp + tan(3 * f->real_tmp)));
		iter++;
	}
	return (iter);
}

int			draw_popcorn(t_fractol *f, int y, int x)
{
	int			iter;

	f->y = (y - W_HEIGHT / 2) * (f->zoom / W_WIDTH) - f->ky;
	f->x = (x - W_WIDTH / 2) * (f->zoom / W_WIDTH) - f->kx;
	if (f->rotation)
		rotate_points_double(&f->x, &f->y, f->rotation);
	f->real = f->y;
	f->im = f->x;
	iter = get_iter(f);
	if (iter == f->iterations)
		return (0);
	if (f->smooth)
		return (get_smooth_color(f, iter));
	return (get_color(f, iter));
}
